...
continuous linear maps `V.orthogonalProjection`, `V⊥.orthogonalProjection`,
...
lemma optimalityEqn_contDiff
  ...
  exact ((V.orthogonalProjection : E →L[ℝ] V).contDiff.comp hres).add
    ((hDφ_adj.comp contDiff_snd).clm_apply
      ((V.orthogonal.orthogonalProjection : E →L[ℝ] V.orthogonal).contDiff.comp hres))

/-- At the base point `(0, 0)`, `∂F/∂v = −Id_V`.

More precisely: `F(r, v) = T_v*(r − v − φ(v))` and at `v = 0`:
  - `T_0 = ι_V` (since `Dφ(0) = 0`), so `T_0* = V.orthogonalProjection`
...
lemma optimalityEqn_partial_v_eq_neg_id
  ...
  have hres : HasFDerivAt (fun v : V => (0 : E) - (v : E) - (φ v : E)) (-ιV) 0 := by
    have h_vE : HasFDerivAt (fun v : V => (v : E) + (φ v : E)) (ιV : V →L[ℝ] E) 0 := by
      simpa using ιV.hasFDerivAt.add h_φE
  ...
  have hterm1 : HasFDerivAt
      (fun v : V => V.orthogonalProjection ((0 : E) - (v : E) - (φ v : E)))
      (-ContinuousLinearMap.id ℝ V) 0 := by
    have h := (V.orthogonalProjection : E →L[ℝ] V).hasFDerivAt.comp (0 : V) hres
    have h_comp_eq : (V.orthogonalProjection : E →L[ℝ] V).comp (-ιV) =
        -ContinuousLinearMap.id ℝ V := by
      ext v
  ...
  have hterm2 : HasFDerivAt
      (fun v : V =>
        ((ContinuousLinearMap.adjoint (fderiv ℝ φ v) : V.orthogonal →L[ℝ] V)
          ((V.orthogonal.orthogonalProjection : E →L[ℝ] V.orthogonal)
            ((0 : E) - (v : E) - (φ v : E)))))
      (0 : V →L[ℝ] V) 0 := by
    ...
    have hb : HasFDerivAt
        (fun v : V => (V.orthogonal.orthogonalProjection : E →L[ℝ] V.orthogonal)
          ((0 : E) - (v : E) - (φ v : E)))
        ((V.orthogonal.orthogonalProjection : E →L[ℝ] V.orthogonal).comp (-ιV)) 0 :=
      (V.orthogonal.orthogonalProjection : E →L[ℝ] V.orthogonal).hasFDerivAt.comp
        (0 : V) hres
    have hA0 : (ContinuousLinearMap.adjoint (fderiv ℝ φ (0 : V)) :
        V.orthogonal →L[ℝ] V) = 0 := by
      rw [hDφ0]; simp only [map_zero]
    have hb0 : (V.orthogonal.orthogonalProjection : E →L[ℝ] V.orthogonal)
...
lemma localMin_sq_dist_implies_optimalityEqn
  ...
  intro w
  change @inner ℝ V _ (V.orthogonalProjection (res v') +
    Dφ.adjoint (V.orthogonal.orthogonalProjection (res v'))) w = 0
  rw [inner_add_left]
  have hterm1 : @inner ℝ V _ (V.orthogonalProjection (res v')) w =
      @inner ℝ E _ (res v') (w : E) :=
    Submodule.inner_orthogonalProjection_eq_of_mem_right w (res v')
  have hterm2 : @inner ℝ V _
      (Dφ.adjoint (V.orthogonal.orthogonalProjection (res v'))) w =
      @inner ℝ E _ (res v') ((Dφ w : V.orthogonal) : E) := by
    rw [ContinuousLinearMap.adjoint_inner_left]
    exact Submodule.inner_orthogonalProjection_eq_of_mem_right (Dφ w) (res v')
...
private lemma chart_point_isLocalMin {S U : Set E}
  ...
  -- v_0 = V.orthogonalProjection(π(y) - m) = v'
  have hv0_eq_v' : v_0 = v' := by
    have hπ_sub : πy - m = (v_0 : E) + (φ v_0 : E) := by rw [hv0_eq]; abel
    change v_0 = V.orthogonalProjection (πy - m)
    rw [hπ_sub, map_add,
...
lemma tubularProj_contDiffAt_S {S U : Set E}
  ...
  set v_star : E → V := fun y => hIFT.implicitFunction (y - m) with hv_def
  have hv_star_cd : ContDiffAt ℝ 1 v_star m := by
    change ContDiffAt ℝ 1 (hIFT.implicitFunction ∘ (· - m)) m
    have hcd : ContDiffAt ℝ 1 hIFT.implicitFunction (m - m) := by
  ...
  have hπ_near_m : ∀ᶠ y in 𝓝 m, π y ∈ Metric.ball m δ := by
    ...
    rwa [hπ_m] at h1
  -- The map y ↦ (y − m, V.orthogonalProjection(π(y) − m)) tends to (0, 0)
  have h_tend_pair : Tendsto (fun y => (y - m, V.orthogonalProjection (π y - m)))
      (𝓝 m) (𝓝 (0, (0 : V))) := by
    ...
    · have h_sub_tend : Tendsto (fun y => π y - m) (𝓝 m) (𝓝 (0 : E)) := by
      ...
      rw [show (0 : V) = (V.orthogonalProjection : E →L[ℝ] V) 0 from (map_zero _).symm]
      exact ((V.orthogonalProjection : E →L[ℝ] V).continuous.tendsto 0).comp h_sub_tend
  ...
  have h_opt_near : ∀ᶠ y in 𝓝 m, y ∈ U → π y ∈ Metric.ball m δ →
      optimalityEqn φ m (y - m, V.orthogonalProjection (π y - m)) = 0 := by
  ...
  have h_chart_repr : ∀ᶠ y in 𝓝 m, y ∈ U → π y ∈ Metric.ball m δ →
      π y = m + (V.orthogonalProjection (π y - m) : E) +
        (φ (V.orthogonalProjection (π y - m)) : E) := by
    apply Eventually.of_forall
    intro y hy_U hπ_ball
    have hπ_S := (tubularProj_mem hTN hne y hy_U).1
    obtain ⟨v_0, hv0_eq⟩ := (hchart (π y) hπ_ball).mp hπ_S
    have hv0_eq_w : v_0 = V.orthogonalProjection (π y - m) := by
  ...
  have hπ_eq_χ : π =ᶠ[𝓝 m] χ := by
    ...
    set w_y := V.orthogonalProjection (π y - m) with hw_def
    ...
    change π y = m + (hIFT.implicitFunction (y - m) : E) +
      (φ (hIFT.implicitFunction (y - m)) : E)
