...
private lemma tubularProj_hasFDerivAt_starProjection {S U : Set E}
  ...
  let χ : E → E := fun x =>
    m + (V.starProjection (x - m) : E) + ((φ (V.orthogonalProjection (x - m))) : E)
  ...
  have h_diff : (fun x => tubularProj hTN hne x - χ x) =o[𝓝 m]
    ...
    have hπ_sub : π_x - m = (v_star : E) + (φ v_star : E) := by
      rw [hπ_eq]; abel
    -- V.orthogonalProjection(π_x - m) = v_star
    have hπ_proj : V.orthogonalProjection (π_x - m) = v_star := by
      rw [hπ_sub, map_add,
    ...
    have hvs_norm : ‖(v_star : E)‖ ≤ 2 * ‖x - m‖ := by
      have h := Submodule.norm_orthogonalProjection_apply_le V (π_x - m)
    ...
    have hχ_sub_m : χ x - m = (v : E) + (φ v : E) := by
      change m + V.starProjection (x - m) + (φ (V.orthogonalProjection (x - m)) : E) - m =
        (v : E) + (φ v : E)
      rw [h_star_eq]; abel
  ...
  have h_chartHasFDeriv : HasFDerivAt χ V.starProjection m := by
    ...
    -- HasFDerivAt (V.orthogonalProjection ∘ (· - m)) at m
    have h_proj : HasFDerivAt (fun x => V.orthogonalProjection (x - m))
        (V.orthogonalProjection.comp (.id ℝ E)) m :=
      (V.orthogonalProjection : E →L[ℝ] V).hasFDerivAt.comp m h_sub
    -- HasFDerivAt (φ ∘ V.orthogonalProjection ∘ (· - m)) at m with derivative 0
    have h_phi : HasFDerivAt (fun x => φ (V.orthogonalProjection (x - m)))
    ...
    have h_coe : HasFDerivAt (fun x => (φ (V.orthogonalProjection (x - m)) : E))
        (0 : E →L[ℝ] E) m := by
